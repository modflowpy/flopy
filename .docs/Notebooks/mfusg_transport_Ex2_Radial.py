...
for icol in range(ncol):
    lrcsc.append([0,0,icol,20.,20.])
for irow in range(1, nrow-1):
    lrcsc.append([0,irow,0,20.,20.])
    lrcsc.append([0,irow,99,20.,20.])
for icol in range(ncol):
    lrcsc.append([0,99,icol,20.,20.])
...
dl = 500.0
dt = 50.0
bct = MfUsgBct(mf,ipakcb=55, itvd=8,cinact=-999.0, diffnc=0.0, prsity=prsity, dl = dl, dt = dt, conc = conc)
...
mf.remove_package("BCT")
bct = MfUsgBct(mf,ipakcb=55, itvd=8,cinact=-999.0, diffnc=0.0, ixdisp=1, prsity=prsity, dl = dl, dt = dt, conc = conc)
