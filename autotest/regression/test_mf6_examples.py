...
def test_mf6_example_simulations(tmpdir, mf6_example_namfiles):
    ...
    if len(mf6_example_namfiles) == 0: pytest.skip("No namfiles (expected ordered collection)")
    ...
    tmpdir = Path(tmpdir / "workspace")  # working directory (must not exist for copytree)
    ...
    def run_models():
        ...
        for namfile in mf6_example_namfiles:
            ...
            # load simulation
            sim = MFSimulation.load(namfile_name,
                                    version="mf6",
                                    exe_name="mf6",
                                    sim_ws=str(wrkdir))
