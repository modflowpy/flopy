...
def test_swr_binary_stage(ipos=0):
    ...
    assert isinstance(sobj, flopy.utils.SwrStage), 'SwrStage object not created'
...
def test_swr_binary_structure(ipos=4):
    ...
    assert isinstance(sobj, flopy.utils.SwrStructure), 'SwrStructure object not created'
...
def test_swr_binary_obs(ipos=5):
    ...
    ts = sobj.get_data()
    assert ts.shape == (
    336, 1), 'SwrObs length of data array does not equal (336, 1)'
    assert len(
