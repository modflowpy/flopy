...
def build_model():
	...
	model_ws = "temp"
	ibound_path = os.path.join("..","examples","data","subwt_example","ibound.ref")


	ml = flopy.modflow.Modflow("subwt_mf2005",model_ws=model_ws)
	perlen = [1.0,60. * 365.25, 60 * 365.25]
	nstp = [1,60,60]
	flopy.modflow.ModflowDis(ml, nlay=4, nrow=20, ncol=15, delr=2000.0, delc=2000.0,
	                         nper=3, steady=[True, False, False],perlen=perlen,nstp=nstp,
	                         top=150.0, botm=[50, -100, -150.0, -350.0])

	flopy.modflow.ModflowLpf(ml,laytyp=[1,0,0,0],hk=[4,4,0.01,4],vka=[0.4,0.4,0.01,0.4],
	                         sy=0.3,ss=1.0e-6)
	...
	flopy.modflow.ModflowWel(ml,stress_period_data={0:sp1_wells,1:sp2_wells,2:sp1_wells})
