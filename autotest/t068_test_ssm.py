...
def test_mt3d_ssm_with_nodata_in_1st_sp():
    ...
    if run:
        ...
        success, buff = mt.run_model(
            report=True, normal_msg="Program completed."
        )
        ...
        mt2 = flopy.mt3d.Mt3dms.load(
            "model_mt.nam", model_ws=model_ws, verbose=True, exe_name="mt3dms"
        )
...
def test_none_spdtype():
    ...
    mf.write_input()
    mf2 = flopy.modflow.Modflow.load(
        "modflowtest.nam", model_ws=model_ws, verbose=True
    )
    if run:
        success, buff = mf.run_model(report=True)
        assert success
...
def test_ssm_readwrite():
    ...
    mt = flopy.mt3d.Mt3dms(
        modflowmodel=mf, modelname="tran_v1", version="mt3d-usgs"
    )
