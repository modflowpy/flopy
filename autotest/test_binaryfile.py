...
def test_headfile_write_roundtrip(tmp_path, example_data_path):
    """Test HeadFile.write_head() with roundtrip validation."""
    # Read an existing head file
    original_file = example_data_path / "mf6-freyberg" / "freyberg.hds"
    original = HeadFile(original_file)

    # Extract all data
    data_dict = {}
    for idx, kstpkper_np in enumerate(original.kstpkper):
        kstpkper = (int(kstpkper_np[0]), int(kstpkper_np[1]))
        head = original.get_data(idx=idx)
        totim = original.times[idx]
        data_dict[kstpkper] = {"head": head, "pertim": totim, "totim": totim}

    # Write to a temp file
    output_file = tmp_path / "test.hds"
    HeadFile.write_head(
        output_file, data_dict, text="HEAD", precision="double", verbose=False
    )

    # Read it back
    readback = HeadFile(output_file)

    # Compare
    assert len(readback.kstpkper) == len(original.kstpkper), (
        f"Number of time steps mismatch: "
        f"{len(readback.kstpkper)} != {len(original.kstpkper)}"
    )

    for idx in range(len(original.kstpkper)):
        orig_data = original.get_data(idx=idx)
        read_data = readback.get_data(idx=idx)
        np.testing.assert_allclose(
            orig_data, read_data, err_msg=f"Head data mismatch at idx={idx}"
        )


def test_headfile_write_synthetic(tmp_path):
    """Test HeadFile.write_head() with synthetic data."""
    nlay, nrow, ncol = 2, 10, 15

    # Create synthetic head data for multiple time steps
    data_dict = {}
    for kper in range(1, 3):
        for kstp in range(1, 3):
            heads = np.random.rand(nlay, nrow, ncol) * 10.0 + 100.0
            totim = (kper - 1) * 10.0 + kstp
            data_dict[(kstp, kper)] = {
                "head": heads,
                "pertim": float(kstp),
                "totim": totim,
    ...
    # Write the file
    output_file = tmp_path / "synthetic.hds"
    HeadFile.write_head(output_file, data_dict, text="HEAD", precision="single")

    # Read it back
    hf = HeadFile(output_file, precision="single")

    assert len(hf.kstpkper) == 4, f"Expected 4 time steps, got {len(hf.kstpkper)}"

    # Verify data roundtrip
    for (kstp, kper), entry in data_dict.items():
        data_back = hf.get_data(kstpkper=(kstp - 1, kper - 1))
        np.testing.assert_allclose(
            entry["head"], data_back, err_msg=f"Mismatch at kstp={kstp}, kper={kper}"
        )


def test_cellbudgetfile_write_array_format(tmp_path):
    """Test CellBudgetFile.write_budget() with array format (imeth=1)."""
    from flopy.mf6.utils.binarygrid_util import build_structured_connectivity

    nlay, nrow, ncol = 1, 10, 20
    ncells = nlay * nrow * ncol

    # Build connectivity for this grid to get NJA
    ia, ja, nja = build_structured_connectivity(nlay, nrow, ncol)

    # Create synthetic FLOW-JA-FACE data (size NJA, not ncells)
    flowja = np.random.rand(nja) - 0.5
    ...
    # Write the budget file
    output_file = tmp_path / "test_array.bud"
    ...
    # Read it back
    cbb = CellBudgetFile(output_file, precision="double")

    assert len(cbb.textlist) == 1, f"Expected 1 budget term, got {len(cbb.textlist)}"
    # Text is padded to 16 characters in binary format
    assert "FLOW-JA-FACE".ljust(16).encode() in cbb.textlist, (
        "FLOW-JA-FACE not in budget file"
    )

    # Get data back
    data_back = cbb.get_data(kstpkper=(0, 0), text="FLOW-JA-FACE")[0]

    # FLOW-JA-FACE is returned as 3D array with shape (1, 1, NJA)
    # This matches how real MF6 files are structured
    assert data_back.shape == (1, 1, nja), (
        f"Expected shape (1, 1, {nja}), got {data_back.shape}"
    )
    np.testing.assert_allclose(
        flowja, data_back.flatten(), err_msg="FLOW-JA-FACE data mismatch"
    )


def test_cellbudgetfile_write_array_format_regular(tmp_path):
    """Test CellBudgetFile.write_budget() with array format for regular budget term."""
    nlay, nrow, ncol = 2, 5, 10
    ncells = nlay * nrow * ncol

    # Create synthetic budget data (e.g., storage)
    storage = np.random.rand(nlay, nrow, ncol) * 100.0
    ...
    # Write the budget file
    output_file = tmp_path / "test_regular.bud"
    CellBudgetFile.write_budget(
        output_file,
        budget_dict,
        nlay=nlay,
    ...
    assert "STORAGE".ljust(16).encode() in cbb.textlist, "STORAGE not in budget file"

    # Get data back
    data_back = cbb.get_data(kstpkper=(0, 0), text="STORAGE")[0]

    # Regular budget terms are returned as 3D arrays
    assert data_back.shape == (
        nlay,
        nrow,
        ncol,
    ), f"Expected shape {(nlay, nrow, ncol)}, got {data_back.shape}"
    np.testing.assert_allclose(storage, data_back, err_msg="STORAGE data mismatch")


def test_cellbudgetfile_write_list_format(tmp_path):
    """Test CellBudgetFile.write_budget() with list format (imeth=6)."""
    nlay, nrow, ncol = 1, 10, 20
    ncells = nlay * nrow * ncol

    # Create synthetic DATA-SPDIS list data
    dt = np.dtype(
        [
            ("node", np.int32),
            ("node2", np.int32),
    ...
    data_list["q"] = 0.0
    data_list["qx"] = np.random.rand(ncells) * 0.001
    data_list["qy"] = np.random.rand(ncells) * 0.001
    data_list["qz"] = np.random.rand(ncells) * 0.001
    ...
    # Write the budget file
    output_file = tmp_path / "test_list.bud"
    CellBudgetFile.write_budget(
        output_file,
        budget_dict,
        nlay=nlay,
    ...
    # Text is padded to 16 characters in binary format
    assert "DATA-SPDIS".ljust(16).encode() in cbb.textlist, (
        "DATA-SPDIS not in budget file"
    )

    # Get data back
    data_back = cbb.get_data(kstpkper=(0, 0), text="DATA-SPDIS")[0]

    assert len(data_back) == ncells, f"Expected {ncells} entries, got {len(data_back)}"
    np.testing.assert_allclose(data_list["qx"], data_back["qx"], err_msg="qx mismatch")
    np.testing.assert_allclose(data_list["qy"], data_back["qy"], err_msg="qy mismatch")
    np.testing.assert_allclose(data_list["qz"], data_back["qz"], err_msg="qz mismatch")


def test_cellbudgetfile_write_multiple_terms(tmp_path):
    """Test CellBudgetFile.write_budget() with multiple budget terms."""
    from flopy.mf6.utils.binarygrid_util import build_structured_connectivity
    ...
    # Create FLOW-JA-FACE array data (size NJA)
    flowja = np.random.rand(nja) - 0.5

    # Create DATA-SAT list data
    dt_sat = np.dtype([("node", np.int32), ("node2", np.int32), ("q", np.float64)])
    sat_list = np.zeros(ncells, dtype=dt_sat)
    sat_list["node"] = np.arange(1, ncells + 1)
    sat_list["node2"] = np.arange(1, ncells + 1)
    sat_list["q"] = np.random.rand(ncells)
    ...
    # Write the budget file
    output_file = tmp_path / "test_multi.bud"
    CellBudgetFile.write_budget(
        output_file,
        budget_dict,
        nlay=nlay,
    ...
    assert len(cbb.textlist) == 2, f"Expected 2 budget terms, got {len(cbb.textlist)}"

    # Verify FLOW-JA-FACE (returned as shape (1, 1, NJA))
    flowja_back = cbb.get_data(kstpkper=(0, 0), text="FLOW-JA-FACE")[0]
    assert flowja_back.shape == (1, 1, nja), (
        f"Expected shape (1, 1, {nja}), got {flowja_back.shape}"
    )
    np.testing.assert_allclose(flowja, flowja_back.flatten())

    # Verify DATA-SAT
    sat_back = cbb.get_data(kstpkper=(0, 0), text="DATA-SAT")[0]
    np.testing.assert_allclose(sat_list["q"], sat_back["q"])


def test_headfile_instance_write(tmp_path):
    """Test HeadFile.write() instance method."""
    # Create test data
    data_dict = {
        (1, 1): {
    ...
    # Write initial file
    file1 = tmp_path / "test.hds"
    HeadFile.write_head(file1, data_dict, precision="double")

    # Read it back and write using instance method
    hds = HeadFile(file1, precision="double")
    file2 = tmp_path / "test_copy.hds"
    hds.write(file2)
    hds.close()

    # Verify the copy
    hds2 = HeadFile(file2, precision="double")
    assert len(hds2.kstpkper) == 2
    head1 = hds2.get_data(totim=1.0)
    head2 = hds2.get_data(totim=2.0)
    np.testing.assert_allclose(head1, 35.0)
    np.testing.assert_allclose(head2, 34.0)
    hds2.close()


def test_headfile_instance_write_filtered(tmp_path):
    """Test HeadFile.write() with kstpkper filtering."""
    # Create test data with 3 time steps
    data_dict = {
        (1, 1): {"head": np.full((2, 5, 5), 35.0), "pertim": 1.0, "totim": 1.0},
        (2, 1): {"head": np.full((2, 5, 5), 34.0), "pertim": 2.0, "totim": 2.0},
        (3, 1): {"head": np.full((2, 5, 5), 33.0), "pertim": 3.0, "totim": 3.0},
    }
    ...
    # Read and write only specific time steps
    hds = HeadFile(file1, precision="double")
    file2 = tmp_path / "test_filtered.hds"
    hds.write(file2, kstpkper=[(1, 1), (3, 1)])  # Skip (2, 1)
    hds.close()

    # Verify only 2 time steps in output
    hds2 = HeadFile(file2, precision="double")
    assert len(hds2.kstpkper) == 2
    assert (1, 1) in hds2.kstpkper
    assert (3, 1) in hds2.kstpkper
    assert (2, 1) not in hds2.kstpkper
    hds2.close()


def test_cellbudgetfile_instance_write(tmp_path):
    """Test CellBudgetFile.write() instance method."""
    from flopy.mf6.utils.binarygrid_util import build_structured_connectivity
    ...
    # Write initial file (no grid dimensions needed for FLOW-JA-FACE only)
    file1 = tmp_path / "test.cbc"
    CellBudgetFile.write_budget(file1, budget_dict, precision="double")

    # Read and write using instance method
    cbb = CellBudgetFile(file1, precision="double")
    file2 = tmp_path / "test_copy.cbc"
    cbb.write(file2)
    cbb.close()

    # Verify the copy
    cbb2 = CellBudgetFile(file2, precision="double")
    assert len(cbb2.kstpkper) == 1
    # get_data uses 0-based indexing
    flowja_back = cbb2.get_data(kstpkper=(0, 0), text="FLOW-JA-FACE")[0]
    np.testing.assert_allclose(flowja, flowja_back.flatten())
    cbb2.close()


def test_cellbudgetfile_instance_write_filtered(tmp_path):
    """Test CellBudgetFile.write() with text filtering."""
    from flopy.mf6.utils.binarygrid_util import build_structured_connectivity

    nlay, nrow, ncol = 1, 10, 20
    ia, ja, nja = build_structured_connectivity(nlay, nrow, ncol)

    # Create budget data with two terms
    flowja = np.random.rand(nja) - 0.5
    storage = np.random.rand(nlay, nrow, ncol)
    ...
    file1 = tmp_path / "test.cbc"
    CellBudgetFile.write_budget(
        file1, budget_dict, nlay=nlay, nrow=nrow, ncol=ncol, precision="double"
    )

    # Read and write only FLOW-JA-FACE
    cbb = CellBudgetFile(file1, precision="double")
    file2 = tmp_path / "test_filtered.cbc"
    cbb.write(file2, text="FLOW-JA-FACE")
    cbb.close()

    # Verify only FLOW-JA-FACE in output
    cbb2 = CellBudgetFile(file2, precision="double")
    assert len(cbb2.textlist) == 1
    assert b"FLOW-JA-FACE" in cbb2.textlist[0]
    # get_data uses 0-based indexing
    flowja_back = cbb2.get_data(kstpkper=(0, 0), text="FLOW-JA-FACE")[0]
    np.testing.assert_allclose(flowja, flowja_back.flatten())
    cbb2.close()
