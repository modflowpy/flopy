...
def test_gage(tmpdir):
    mnam = "gage_test"

    m = Modflow(modelname=mnam, model_ws=str(tmpdir))
    dis = ModflowDis(m)
    spd = {
        (0, 0): ["print head"],
        (0, 1): [],
    ...
    fpth = os.path.join(str(tmpdir), f"{mnam}.nam")
...
def test_gage_filenames0(tmpdir):
...
def test_gage_filenames(tmpdir):
