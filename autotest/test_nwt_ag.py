...
def test_empty_ag_package(tmpdir):
    ml = Modflow("agtest", version="mfnwt")
    ag = ModflowAg(ml)
    assert isinstance(ag, Package)


def test_load_write_agwater(tmpdir, example_data_path):
    agfile = "Agwater1.ag"
    ...
    ag2 = ModflowAg.load(str(tmpdir / agfile), ml2, nper=49)

    assert repr(ag1) == repr(ag2), "comparison failed"


def test_load_write_agwater_uzf(tmpdir, example_data_path):
    ...
    uzf2 = ModflowUzf1.load(str(tmpdir / uzffile), ml2)
