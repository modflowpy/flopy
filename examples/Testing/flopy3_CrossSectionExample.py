...
fb = modelxsect.plot_fill_between(
    head, colors=["brown", "cyan"], masked_values=[999.00]
)
...
cr = np.linspace(10.0, 25.0, num=cmap.N)
norm = matplotlib.colors.BoundaryNorm(cr, cmap.N)
hv = modelxsect.plot_array(
    head, head=head, masked_values=[999.00, -1.00000e30]
)  # , norm=norm)
...
# which could be manipulated (or used) later if necessary.
hv = modelxsect.plot_array(
    head, masked_values=[999.00, -1.00000e30], norm=norm
)
ct = modelxsect.contour_array(
    head,
    masked_values=[999.00, -1.00000e30],
    colors="black",
    linewidths=0.5,
