...
ztop = 30.0  # top of layer (ft rel to msl)
botm = -1000.0  # bottom of layer (ft rel to msl)
...
lpf = flopy.modflow.ModflowLpf(
    mf, hk=hk, sy=sy, ss=ss, laytyp=laytyp, layavg=layavg
)
...
lrcq = np.vstack(
    (lrcq, [0, 8, 7, -90 * 133680], [0, 10, 9, -80 * 133680])
)  # add wells (row/col, zero-based)
# print lrcq
...
oc = flopy.modflow.ModflowOc88(
    mf, save_head_every=100, item2=[[0, 1, 0, 0]], item3=[[0, 1, 0, 0]]
)
pcg = flopy.modflow.ModflowPcg(
    mf, hclose=1.0e-4, rclose=5.0e-0
...
m2 = flopy.modflow.Modflow.load(
    modelname, exe_name="mf2005", model_ws=workspace, verbose=True
)
