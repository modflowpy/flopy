...
ml = flopy.modflow.Modflow(modelname, version='mf2005', exe_name=exe_name,
                           model_ws=workspace)
discret = flopy.modflow.ModflowDis(ml, nrow=nrow, ncol=ncol, nlay=3, delr=delr,
                                   delc=delc,
...
swi = flopy.modflow.ModflowSwi2(ml, iswizt=55, nsrf=1, istrat=1,
                                toeslope=0.01, tipslope=0.04, nu=[0, 0.025],
                                zeta=[zini, zini, zini], ssz=0.2, isource=iso,
                                nsolver=1)
...
ax.text(-0.075, 1.05, 'C', transform=ax.transAxes, va='center', ha='center',
        size='8')
# confining unit
ax.fill([-600, 3400, 3400, -600], [-29, -29, -30, -30], fc=[.8, .8, .8],
        ec=[.8, .8, .8])
#
zt = MergeData(ncol, [zeta[4, 0, 0, :], zeta[4, 1, 0, :], zeta[4, 2, 0, :]],
               zedge)
ax.plot(x, zt, marker='o', markersize=3, linewidth=0.0, markeredgecolor='blue',
