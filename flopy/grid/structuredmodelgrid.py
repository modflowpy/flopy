import numpy as np
from flopy.grid.modelgrid import ModelGrid, CachedData, CachedDataType
...
class StructuredModelGrid(ModelGrid):
    ...
    def xll(self):
        if self._origin_loc == 'll':
            ...
            xll = self._origin_x - (np.sin(-self.rotation_radians) * self.yedges[0] *
    ...
    def yll(self):
        if self._origin_loc == 'll':
            ...
            # calculate coords for lower left corner
            yll = self._origin_y - (np.cos(-self.rotation_radians) *
                                    self.yedges[0] * self.length_multiplier)
        else:
            raise Exception('Invalid origin location "{}".'.format(
                self._origin_loc))
        return yll
    ...
    def xul(self):
        if self._origin_loc == 'll':
            ...
            xul = self._origin_x + (np.sin(-self.rotation_radians) *
                                    self.yedges[0] * self.length_multiplier)
        elif self._origin_loc == 'ul':
            # calculate coords for lower left corner
    ...
    def yul(self):
        if self._origin_loc == 'll':
            # calculate coords for upper left corner
            yul = self._origin_y + (np.cos(-self.rotation_radians) *
                                    self.yedges[0] * self.length_multiplier)
        elif self._origin_loc == 'ul':
            # calculate coords for lower left corner
