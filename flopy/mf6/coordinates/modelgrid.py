...
class UnstructuredModelCell(ModelCell):
    ...
    set_top : (top_elv : float, update_connections : boolean)
        sets the top elevation of the model cell and updates the connection properties if
        update_connections is true
    set_bot : (bot_elv : float, update_connections : boolean)
        sets the bottom elevation of the model cell and updates the connection properties if
        update_connections is true
    set_area : (area : float)
        sets the area of the model cell
    add_connection : (to_cellid, ihc_direction, connection_length, connection_area, connection_angle=0)
        adds a connection from this cell to the cell with ID to_cellid
        connection properties ihc_direction, connection_length, connection_area, and connection_angle
    ...
    def get_top(self):
        tops = self._simulation_data.mfdata[(self._model_name, 'DISU8', 'DISDATA', 'top')]
        return tops[self._cellid-1]

    def get_bot(self):
        bots = self._simulation_data.mfdata[(self._model_name, 'DISU8', 'DISDATA', 'bot')]
        return bots[self._cellid-1]

    def get_area(self):
        areas = self._simulation_data.mfdata[(self._model_name, 'DISU8', 'DISDATA', 'area')]
        return areas[self._cellid-1]

    def get_num_connections_iac(self):
        iacs = self._simulation_data.mfdata[(self._model_name, 'DISU8', 'CONNECTIONDATA', 'iac')]
        return iacs[self._cellid-1]

    def get_connecting_cells_ja(self):
        jas = self._simulation_data.mfdata[(self._model_name, 'DISU8', 'CONNECTIONDATA', 'ja')]
        return jas[self._cellid-1]

    def get_connection_direction_ihc(self):
        ihc = self._simulation_data.mfdata[(self._model_name, 'DISU8', 'CONNECTIONDATA', 'ihc')]
        return ihc[self._cellid-1]

    def get_connection_length_cl12(self):
        cl12 = self._simulation_data.mfdata[(self._model_name, 'DISU8', 'CONNECTIONDATA', 'cl12')]
        return cl12[self._cellid-1]

    def get_connection_area_fahl(self):
        fahl = self._simulation_data.mfdata[(self._model_name, 'DISU8', 'CONNECTIONDATA', 'fahl')]
        return fahl[self._cellid-1]

    def get_connection_anglex(self):
        anglex = self._simulation_data.mfdata[(self._model_name, 'DISU8', 'CONNECTIONDATA', 'anglex')]
        return anglex[self._cellid-1]

    def set_top(self, top_elv, update_connections=True):
        tops = self._simulation_data.mfdata[(self._model_name, 'DISU8', 'DISDATA', 'top')]
        if update_connections:
            self._update_connections(self.get_top(), top_elv, self.get_bot(), self.get_bot())
        tops[self._cellid-1] = top_elv

    def set_bot(self, bot_elv, update_connections=True):
        bots = self._simulation_data.mfdata[(self._model_name, 'DISU8', 'DISDATA', 'bot')]
        if update_connections:
            self._update_connections(self.get_top(), self.get_top(), self.get_bot(), bot_elv)
    ...
    def add_connection(self, to_cellid, ihc_direction, connection_length, connection_area, connection_angle=0):
        iacs = self._simulation_data.mfdata[(self._model_name, 'DISU8', 'CONNECTIONDATA', 'iac')]
        jas = self._simulation_data.mfdata[(self._model_name, 'DISU8', 'CONNECTIONDATA', 'ja')]
        ihc = self._simulation_data.mfdata[(self._model_name, 'DISU8', 'CONNECTIONDATA', 'ihc')]
        cl12 = self._simulation_data.mfdata[(self._model_name, 'DISU8', 'CONNECTIONDATA', 'cl12')]
    ...
    def _get_connection_number(self, cellid, reverse_connection=False):
        ...
        # search
        for connection_number, list_cellid in zip(range(0,len(connection_list)), connection_list):
    ...
    def _update_connections(self, old_top_elv, new_top_elv, old_bot_elv, new_bot_elv):
        ...
        for con_number, connecting_cell in zip(range(0,len(jas[self._cellid])), jas[self._cellid-1]):
...
class ModelGrid(object):
    ...
    grid_type : enumeration
        type of model grid (DiscritizationType.DIS, DiscritizationType.DISV, DiscritizationType.DISU)
    ...
    grid_connections_array : ()
        for DiscritizationType.DISU grids, returns an array containing the number of
        connections of it cell
    ...
    get_horizontal_cross_section_dim_names : ()
        returns the appropriate dimension axis for a horizontal cross section based on the model discritization type
    get_model_dim_names : ()
        returns the names of the model dimensions based on the model discritization type
    get_num_spatial_coordinates : ()
        returns the number of spatial coordinates based on the model discritization type
    num_rows
        returns the number of model rows.  model discritization type must be DIS
    num_columns
        returns the number of model columns.  model discritization type must be DIS
    num_connections
        returns the number of model connections.  model discritization type must be DIS
    num_cells_per_layer
        returns the number of cells per model layer.  model discritization type must be DIS or DISV
    num_layers
        returns the number of layers in the model
    num_cells
        returns the total number of cells in the model
    get_all_model_cells
        returns a list of all model cells, represented as a layer/row/column tuple, a layer/cellid tuple, or
        a cellid for the DIS, DISV, and DISU discritizations, respectively
    ...
    def grid_type(self):
        if self.freeze_grid:
            ...
            return coordinateutils.get_grid_type(self._simulation_data, self._model_name)

    def grid_type_consistent(self):
        return self.grid_type() == self._grid_type

    def get_connections_array(self):
        if self.grid_type() == modeldimensions.DiscretizationType.DISU:
            return np.arange(1, self.num_connections() + 1, 1, np.int)
        else:
            except_str = 'ERROR: Can not get connections arrays for model "{}" ' \
                         'Only DISU (unstructured) grids support connections.'.format(self._model_name)
            print(except_str)
            raise MFGridException(except_str)

    def get_horizontal_cross_section_dim_arrays(self):
        if self.grid_type() == modeldimensions.DiscretizationType.DIS:
            return [np.arange(1, self.num_rows() + 1, 1, np.int),
                    np.arange(1, self.num_columns() + 1, 1, np.int)]
        elif self.grid_type() == modeldimensions.DiscretizationType.DISV:
            return [np.arange(1, self.num_cells_per_layer() + 1, 1, np.int)]
        elif self.grid_type() == modeldimensions.DiscretizationType.DISU:
            except_str = 'ERROR: Can not get horizontal plane arrays for model "{}" ' \
                         'DISU grid.  DISU grids do not support individual ' \
    ...
    def get_horizontal_cross_section_dim_names(self):
        if self.grid_type() == modeldimensions.DiscretizationType.DIS:
            ...
            except_str = 'ERROR: Can not get layer dimension name for model "{}" DISU grid. ' \
                         ' DISU grids do not support layers.'.format(self._model_name)
            print(except_str)
            raise MFGridException(except_str)
    ...
    def num_rows(self):
        if self.grid_type() != modeldimensions.DiscretizationType.DIS:
            except_str = 'ERROR: Model "{}" does not have rows.  Can not return ' \
                         'number of rows.'.format(self._model_name)
            print(except_str)
            raise MFGridException(except_str)

        return self._simulation_data.mfdata[(self._model_name, 'dis', 'dimensions', 'nrow')].get_data()

    def num_columns(self):
        if self.grid_type() != modeldimensions.DiscretizationType.DIS:
            except_str = 'ERROR: Model "{}" does not have columns.  Can not return ' \
                         'number of columns.'.format(self._model_name)
            print(except_str)
            raise MFGridException(except_str)

        return self._simulation_data.mfdata[(self._model_name, 'dis', 'dimensions', 'ncol')].get_data()

    def num_connections(self):
        if self.grid_type() == modeldimensions.DiscretizationType.DISU:
            return self._simulation_data.mfdata[(self._model_name, 'disu', 'dimensions', 'nja')].get_data()
        else:
            except_str = 'ERROR: Can not get number of connections for model "{}" ' \
                         'Only DISU (unstructured) grids support connections.'.format(self._model_name)
            print(except_str)
            raise MFGridException(except_str)
    ...
    def num_cells_per_layer(self):
        if self.grid_type() == modeldimensions.DiscretizationType.DIS:
            ...
            return self._simulation_data.mfdata[(self._model_name, 'disv', 'dimensions', 'ncpl')].get_data()
        elif self.grid_type() == modeldimensions.DiscretizationType.DISU:
            except_str = 'ERROR: Model "{}" is unstructured and does not have a consistant number ' \
                         'of cells per layer.'.format(self._model_name)
            print(except_str)
            raise MFGridException(except_str)

    def num_layers(self):
        if self.grid_type() == modeldimensions.DiscretizationType.DIS:
            return self._simulation_data.mfdata[(self._model_name, 'dis', 'dimensions', 'nlay')].get_data()
        elif self.grid_type() == modeldimensions.DiscretizationType.DISV:
            return self._simulation_data.mfdata[(self._model_name, 'disv', 'dimensions', 'nlay')].get_data()
        elif self.grid_type() == modeldimensions.DiscretizationType.DISU:
            return None
    ...
    def num_cells(self):
        if self.grid_type() == modeldimensions.DiscretizationType.DIS:
            ...
            return self._simulation_data.mfdata[(self._model_name, 'disu', 'dimensions', 'nodes')].get_data()
...
class UnstructuredModelGrid(ModelGrid):
    ...
    def __init__(self, model_name, simulation_data):
        super(UnstructuredModelGrid, self).__init__(model_name, simulation_data,
                                                    modeldimensions.DiscretizationType.DISU)
