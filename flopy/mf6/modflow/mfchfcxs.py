...
class ModflowChfcxs(mfpackage.MFPackage):
    """
    ModflowChfcxs defines a cxs package within a chf6 model.
    ...
    packagedata : [idcxs, nxspoints]
        ...
        * nxspoints (integer) integer value that defines the number of points
          ...
          greater than 1 or the program will terminate with an error. NXSPOINTS
          defines the number of points that must be entered for the reach in
          the CROSSSECTIONDATA block. The sum of NXSPOINTS for all sections
          must equal the NPOINTS dimension.
    ...
    packagedata = ListTemplateGenerator(("chf6", "cxs", "packagedata", "packagedata"))
    crosssectiondata = ListTemplateGenerator(
        ("chf6", "cxs", "crosssectiondata", "crosssectiondata")
    )
    package_abbr = "chfcxs"
    _package_type = "cxs"
    dfn_file_name = "chf-cxs.dfn"
