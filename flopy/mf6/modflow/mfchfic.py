...
class ModflowChfic(mfpackage.MFPackage):
    """
    ModflowChfic defines a ic package within a chf6 model.
    ...
    strt : [double]
        * strt (double) is the initial (starting) stage---that is, stage at the
          beginning of the CHF Model simulation. STRT must be specified for all
          CHF Model simulations. One value is read for every model reach.
    filename : String
    ...
    """
    strt = ArrayTemplateGenerator(('chf6', 'ic', 'griddata', 'strt'))
    package_abbr = "chfic"
    _package_type = "ic"
    dfn_file_name = "chf-ic.dfn"
