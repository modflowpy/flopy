...
class ModflowGwfdisu(mfpackage.MFPackage):
    ...
    angldegx : [(angldegx : double)]
        is the angle (in degrees) between the horizontal x-axis and the outward normal to the face between a cell and its connecting cells (see figure 8 in the MODFLOW-USG documentation). The angle varies between zero and 360.0. angldegx is only needed if horizontal anisotropy is specified in the NPF Package. angldegx does not need to be specified if horizontal anisotropy is not used. angldegx is of size nja; values specified for vertical connections and for the diagonal position are not used. Note that angldegx is read in degrees, which is different from MODFLOW-USG, which reads a similar variable (anglex) in radians.
