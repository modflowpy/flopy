...
class ModflowGwfghbg(MFPackage):
    """
    ModflowGwfghbg defines a GHBG package.
    ...
    readarraygrid : keyword
        indicates that array-based grid input will be used for the general-head
        boundary package.  this keyword must be specified to use array-based grid
        input.  when readarraygrid is specified, values must be provided for every cell
        within a model grid, even those cells that have an idomain value less than one.
        values assigned to cells with idomain values less than one are not used and
        have no effect on simulation results. no data cells should contain the value
        dnodata (3.0e+30).
    auxiliary : [string]
    ...
    aux : [double precision]
        ...
        for the auxiliary variable is the same as auxmultname, then the conductance
        array will be multiplied by this array.
