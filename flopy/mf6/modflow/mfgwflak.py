...
class ModflowGwflak(mfpackage.MFPackage):
    ...
    length_conversion : (length_conversion : double)
        length_conversion : real value that is used in converting outlet flow terms that use Manning's equation or gravitational acceleration to consistent length units. length\_conversion should be set to 3.28081, 1.0, and 100.0 when using length units (length\_units) of feet, meters, or centimeters in the simulation, respectively. length\_conversion does not need to be specified if no lake outlets are specified or length\_units are meters.
