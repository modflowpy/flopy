...
class ModflowGwfmaw(MFPackage):
    ...
    packagedata : [(ifno, radius, bottom, strt, condeqn, ngwfnodes, aux, boundname)]
        * ifno : integer
        ...
        * radius : double precision
                radius for the multi-aquifer well. The program will terminate with an error if
                the radius is less than or equal to zero.
        * bottom : double precision
                bottom elevation of the multi-aquifer well. If CONDEQN is SPECIFIED, THIEM,
                SKIN, or CUMULATIVE, BOTTOM is set to the cell bottom in the lowermost GWF cell
                connection in cases where the specified well bottom is above the bottom of this
                GWF cell. If CONDEQN is MEAN, BOTTOM is set to the lowermost GWF cell
                connection screen bottom in cases where the specified well bottom is above this
                value. The bottom elevation defines the lowest well head that will be simulated
                when the NEWTON UNDER_RELAXATION option is specified in the GWF model name
                file. The bottom elevation is also used to calculate volumetric storage in the
                well.
        * strt : double precision
                starting head for the multi-aquifer well. The program will terminate with an
                error if the starting head is less than the specified well bottom.
        * condeqn : string
                character string that defines the conductance equation that is used to
                calculate the saturated conductance for the multi-aquifer well. Possible multi-
                aquifer well CONDEQN strings include: SPECIFIED--character keyword to indicate
                the multi-aquifer well saturated conductance will be specified.  THIEM--
                character keyword to indicate the multi-aquifer well saturated conductance will
                be calculated using the Thiem equation, which considers the cell top and
                bottom, aquifer hydraulic conductivity, and effective cell and well radius.
                SKIN--character keyword to indicate that the multi-aquifer well saturated
                conductance will be calculated using the cell top and bottom, aquifer and
                screen hydraulic conductivity, and well and skin radius.  CUMULATIVE--character
                keyword to indicate that the multi-aquifer well saturated conductance will be
                calculated using a combination of the Thiem and SKIN equations.  MEAN--
                character keyword to indicate the multi-aquifer well saturated conductance will
                be calculated using the aquifer and screen top and bottom, aquifer and screen
                hydraulic conductivity, and well and skin radius. The CUMULATIVE conductance
                equation is identical to the SKIN LOSSTYPE in the Multi-Node Well (MNW2)
                package for MODFLOW-2005. The program will terminate with an error condition if
                CONDEQN is SKIN or CUMULATIVE and the calculated saturated conductance is less
                than zero; if an error condition occurs, it is suggested that the THIEM or MEAN
                conductance equations be used for these multi-aquifer wells.
        * ngwfnodes : integer
                integer value that defines the number of GWF nodes connected to this (IFNO)
                multi-aquifer well. NGWFNODES must be greater than zero.
        * aux : [double precision]
                represents the values of the auxiliary variables for each multi-aquifer well.
                The values of auxiliary variables must be present for each multi-aquifer well.
        ...
        * boundname : string
                name of the multi-aquifer well cell.  BOUNDNAME is an ASCII character variable
    ...
    connectiondata : [(ifno, icon, cellid, scrn_top, scrn_bot, hk_skin, radius_skin)]
        ...
        * cellid : [integer]
                ...
                (DISU) input file, CELLID is the node number for the cell. One or more screened
                intervals can be connected to the same CELLID if CONDEQN for a well is MEAN.
                The program will terminate with an error if MAW wells using SPECIFIED, THIEM,
                SKIN, or CUMULATIVE conductance equations have more than one connection to the
                same CELLID.
        * scrn_top : double precision
                value that defines the top elevation of the screen for the multi-aquifer well
                connection. If CONDEQN is SPECIFIED, THIEM, SKIN, or CUMULATIVE, SCRN_TOP can
                be any value and is set to the top of the cell. If CONDEQN is MEAN, SCRN_TOP is
                set to the multi-aquifer well connection cell top if the specified value is
                greater than the cell top. The program will terminate with an error if the
                screen top is less than the screen bottom.
        * scrn_bot : double precision
                value that defines the bottom elevation of the screen for the multi-aquifer
                well connection. If CONDEQN is SPECIFIED, THIEM, SKIN, or CUMULATIVE, SCRN_BOT
                can be any value and is set to the bottom of the cell. If CONDEQN is MEAN,
                SCRN_BOT is set to the multi-aquifer well connection cell bottom if the
                specified value is less than the cell bottom. The program will terminate with
                an error if the screen bottom is greater than the screen top.
        * hk_skin : double precision
                value that defines the skin (filter pack) hydraulic conductivity (if CONDEQN
                for the multi-aquifer well is SKIN, CUMULATIVE, or MEAN) or conductance (if
                CONDEQN for the multi-aquifer well is SPECIFIED) for each GWF node connected to
                the multi-aquifer well (NGWFNODES). If CONDEQN is SPECIFIED, HK_SKIN must be
                greater than or equal to zero.  HK_SKIN can be any value if CONDEQN is THIEM.
                Otherwise, HK_SKIN must be greater than zero. If CONDEQN is SKIN, the contrast
                between the cell transmissivity (the product of geometric mean horizontal
                hydraulic conductivity and the cell thickness) and the well transmissivity (the
                product of HK_SKIN and the screen thicknesses) must be greater than one in node
                CELLID or the program will terminate with an error condition; if an error
                condition occurs, it is suggested that the HK_SKIN be reduced to a value less
                than K11 and K22 in node CELLID or the THIEM or MEAN conductance equations be
                used for these multi-aquifer wells.
        * radius_skin : double precision
                real value that defines the skin radius (filter pack radius) for the multi-
    ...
    perioddata : [(ifno, mawsetting)]
        * ifno : integer
                ...
                data on the line. ifno must be greater than zero and less than or equal to
                nmawwells.
        * mawsetting : status | flowing_wellrecord | rate | well_head | head_limit | shutoffrecord | rate_scalingrecord | auxiliaryrecord
                line of information that is parsed into a keyword and values.  keyword values
                that can be used to start the mawsetting string include: status, flowing_well,
                rate, well_head, head_limit, shut_off, rate_scaling, and auxiliary.
                * status : string
                            keyword option to define well status.  status can be active, inactive, or
                            constant. by default, status is active.
                * flowing_wellrecord : (flowing_well, fwelev, fwcond, fwrlen)
                            * flowing_well : keyword
                                            keyword to indicate the well is a flowing well.  The FLOWING_WELL option can be
                                            used to simulate flowing wells when the simulated well head exceeds the
                                            specified drainage elevation.
                            * fwelev : double precision
                                            elevation used to determine whether or not the well is flowing.
                            * fwcond : double precision
                                            conductance used to calculate the discharge of a free flowing well.  Flow
                                            occurs when the head in the well is above the well top elevation (FWELEV).
                            * fwrlen : double precision
                                            length used to reduce the conductance of the flowing well.  When the head in
                                            the well drops below the well top plus the reduction length, then the
                                            conductance is reduced.  This reduction length can be used to improve the
                                            stability of simulations with flowing wells so that there is not an abrupt
                                            change in flowing well rates.

                * rate : double precision
                            is the volumetric pumping rate for the multi-aquifer well. a positive value
                            indicates recharge and a negative value indicates discharge (pumping). rate
                            only applies to active (status is active) multi-aquifer wells. if the options
                            block includes a timeseriesfile entry (see the 'time-variable input' section),
                            values can be obtained from a time series by entering the time-series name in
                            place of a numeric value. by default, the rate for each multi-aquifer well is
                            zero.
                * well_head : double precision
                            is the head in the multi-aquifer well. well_head is only applied to constant
                            head (status is constant) and inactive (status is inactive) multi-aquifer
                            wells. if the options block includes a timeseriesfile entry (see the 'time-
                            variable input' section), values can be obtained from a time series by entering
                            the time-series name in place of a numeric value. the program will terminate
                            with an error if well_head is less than the bottom of the well.
                * head_limit : string
                            is the limiting water level (head) in the well, which is the minimum of the
                            well rate or the well inflow rate from the aquifer. head_limit can be applied
                            to extraction wells (rate < 0) or injection wells (rate > 0). head_limit can be
                            deactivated by specifying the text string 'off'. the head_limit option is based
                            on the head_limit functionality available in the mnw2 citep{konikow2009}
                            package for modflow-2005. the head_limit option has been included to facilitate
                            backward compatibility with previous versions of modflow but use of the
                            rate_scaling option instead of the head_limit option is recommended. by
                            default, head_limit is 'off'.
                * shutoffrecord : (shut_off, minrate, maxrate)
                            * shut_off : keyword
                                            keyword for activating well shut off capability.  Subsequent values define the
                                            minimum and maximum pumping rate that a well must exceed to shutoff or
                                            reactivate a well, respectively, during a stress period. SHUT_OFF is only
                                            applied to injection wells (RATE:math:`<0`) and if HEAD_LIMIT is specified (not
                                            set to 'OFF').  If HEAD_LIMIT is specified, SHUT_OFF can be deactivated by
                                            specifying a minimum value equal to zero. The SHUT_OFF option is based on the
                                            SHUT_OFF functionality available in the MNW2 citep{konikow2009} package for
                                            MODFLOW-2005. The SHUT_OFF option has been included to facilitate backward
                                            compatibility with previous versions of MODFLOW but use of the RATE_SCALING
                                            option instead of the SHUT_OFF option is recommended. By default, SHUT_OFF is
                                            not used.
                            * minrate : double precision
                                            is the minimum rate that a well must exceed to shutoff a well during a stress
                                            period. The well will shut down during a time step if the flow rate to the well
                                            from the aquifer is less than MINRATE. If a well is shut down during a time
                                            step, reactivation of the well cannot occur until the next time step to reduce
                                            oscillations. MINRATE must be less than maxrate.
                            * maxrate : double precision
                                            is the maximum rate that a well must exceed to reactivate a well during a
                                            stress period. The well will reactivate during a timestep if the well was
                                            shutdown during the previous time step and the flow rate to the well from the
                                            aquifer exceeds maxrate. Reactivation of the well cannot occur until the next
                                            time step if a well is shutdown to reduce oscillations. maxrate must be greater
                                            than MINRATE.

                * rate_scalingrecord : (rate_scaling, pump_elevation, scaling_length)
                            * rate_scaling : keyword
                                            activate rate scaling.  If RATE_SCALING is specified, both PUMP_ELEVATION and
                                            SCALING_LENGTH must be specified. RATE_SCALING cannot be used with HEAD_LIMIT.
                                            RATE_SCALING can be used for extraction or injection wells.  For extraction
                                            wells, the extraction rate will start to decrease once the head in the well
                                            lowers to a level equal to the pump elevation plus the scaling length.  If the
                                            head in the well drops below the pump elevation, then the extraction rate is
                                            calculated to be zero.  For an injection well, the injection rate will begin to
                                            decrease once the head in the well rises above the specified pump elevation.
                                            If the head in the well rises above the pump elevation plus the scaling length,
                                            then the injection rate will be set to zero.
                            * pump_elevation : double precision
                                            is the elevation of the multi-aquifer well pump (PUMP_ELEVATION).
                                            PUMP_ELEVATION should not be less than the bottom elevation (BOTTOM) of the
                                            multi-aquifer well.
                            * scaling_length : double precision
                                            height above the pump elevation (SCALING_LENGTH).  If the simulated well head
                                            is below this elevation (pump elevation plus the scaling length), then the
    ...
    **kwargs
    ...
    head_filerecord = ListTemplateGenerator(('gwf6', 'maw', 'options', 'head_filerecord'))
    budget_filerecord = ListTemplateGenerator(('gwf6', 'maw', 'options', 'budget_filerecord'))
    budgetcsv_filerecord = ListTemplateGenerator(('gwf6', 'maw', 'options', 'budgetcsv_filerecord'))
    mfrcsv_filerecord = ListTemplateGenerator(('gwf6', 'maw', 'options', 'mfrcsv_filerecord'))
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.mfrcsv_filerecord = self.build_mfdata('mfrcsv_filerecord', mfrcsv_filerecord)
