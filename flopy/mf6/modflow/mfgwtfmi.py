...
class ModflowGwtfmi(mfpackage.MFPackage):
    ...
    packagedata : [flowtype, fname]
        * flowtype (string) is the word GWFBUDGET, GWFHEAD, GWFMOVER or the
          ...
          then the corresponding file must be a budget file. If GWFHEAD is
          specified, the file must be a head file. If GWFGRID is specified, the
          file must be a binary grid file. If an advanced GWF stress package
          name appears then the corresponding file must be the budget file
          saved by a LAK, SFR, MAW or UZF Package.
