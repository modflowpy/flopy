...
class ModflowOlfic(mfpackage.MFPackage):
    """
    ModflowOlfic defines a ic package within a olf6 model.
    ...
    strt : [double]
        * strt (double) is the initial (starting) stage---that is, stage at the
          beginning of the OLF Model simulation. STRT must be specified for all
          OLF Model simulations. One value is read for every model reach.
    filename : String
    ...
    """
    strt = ArrayTemplateGenerator(('olf6', 'ic', 'griddata', 'strt'))
    package_abbr = "olfic"
    _package_type = "ic"
    dfn_file_name = "olf-ic.dfn"
