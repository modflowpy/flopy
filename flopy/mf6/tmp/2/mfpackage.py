...
class MFPackage(PackageInterface):
    ...
    @staticmethod
    def add_netcdf_entries(attrs, mname, pname, data_item, auxiliary=None, mesh=None, nlay=1):
        if auxiliary:
            auxnames = auxiliary
        else:
            auxnames = []
    ...
    def netcdf_attrs(mtype, ptype, auxiliary=None, mesh=None, nlay=1):
        ...
        from .data.mfdataarray import MFArray
        attrs = {}
        p = modflow.mfgwfnpf.ModflowGwfnpf
        sim_struct = MFSimulationStructure()
        d = DfnPackage(p)
        sim_struct.add_package(d, model_file=False)
        npf = sim_struct.package_struct_objs['npf']
        #print(dir(npf))
        for key, block in npf.blocks.items():
            if key != "griddata" and key != "period":
                continue
            #print(key)
            #print(dir(block))
            #print(block.data_structures)
            for d in block.data_structures:
                #print(d)
                if (block.data_structures[d].layered):
                    #print("layered")
                    pass
                if isinstance(block.data_structures[d], MFArray):
                    #print(d)
                    pass

        for package in MFPackage.__subclasses__():
            sim_struct.process_dfn(DfnPackage(package))
            #print(f"RENO added {DfnPackage(package).dfn_file_name}")
            p = DfnPackage(package)
            c, sc = p.dfn_file_name.split('.')[0].split('-')
            #if p.dfn_file_name == 'gwf-npf.dfn':
                #c, sc = p.dfn_file_name.split('.')[0].split('-')
                #print(c)
                #print(sc)
                #print("FOUND NPF")
        ...
        for key, block in pso.blocks.items():
            ...
            for d in block.data_structures:
                ...
                if (block.data_structures[d].netcdf):
                    print(f"adding {d}")
                    MFPackage.add_netcdf_entries(attrs, mtype, ptype, block.data_structures[d], auxiliary, mesh, nlay)

        return_attrs = {}
        for k in list(attrs):
            return_attrs[k] = attrs[k]['attrs']
    ...
    def netcdf_info(self, mesh=None):
        ...
        def add_entries(name):
            iaux = None
            layer = None
            if dataset.structure.layered and mesh == "LAYERED":
                if name == "aux" or name == "auxvar":
                    for n, auxname in enumerate(
                        ...
                        for l in range(self.model_or_sim.modelgrid.nlay):
                            add_entry(name, n, l + 1)
                else:
                    for l in range(self.model_or_sim.modelgrid.nlay):
                        add_entry(name, layer=l + 1)
            else:
                if name == "aux" or name == "auxvar":
                    for n, auxname in enumerate(
                        self.dimensions.get_aux_variables()[0]
            ...
            for dataset in block.datasets.values():
                if isinstance(dataset, mfdataarray.MFArray):
                    for index, data_item in enumerate(
                        ...
                        if (
                            dataset.structure.netcdf and
                            dataset.has_data()
                        ):
                            #add_entries(data_item.name)
                            MFPackage.add_netcdf_entries(
                                attrs,
                                self.model_name,
                                self.package_name,
                                dataset.structure,
