...
class StartingLocationsFile(Package):
    ...
    def __init__(self, model, inputstyle=1, extension="loc", verbose=False, use_pandas=True):
    ...
    def write_file(self, data=None, float_format="{:.8f}"):
        ...
        data["i0"] += 1
        data["j0"] += 1
        if pandas_available and self.use_pandas and len(data) > 0:
    ...
    def _write_particle_data_with_pandas(self, data, float_format):
        ...
        groups = data[['particlegroup', 'groupname']].groupby('particlegroup').count().reset_index().rename(
        ...
        data.to_csv(loc_path, sep=' ', header=False, index=False, mode='a', float_format=float_format)
