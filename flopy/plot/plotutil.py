...
def to_mp7_pathlines(
    ...
    tdis: Optional[List[Tuple[int, int, float]]] = None,
) -> Union[np.recarray, pd.DataFrame]:
    """
    Convert pathline data to MODPATH 7 pathline format. Pathline
    input ``data`` is expected in MODPATH 3, 5, 6, 7, or MODFLOW
    6 PRT format.
    ...
    data : np.recarray or pd.DataFrame
        Tabular pathline data, in MODPATH or MODFLOW 6 PRT format.
    grid : flopy.discretization.grid.Grid
        Optional grid discretization to compute local coordinates,
        if not provided in the input data.
    tdis : list of tuples (int, int, float)
        Optional time discretization to compute stress period and
        time step, if not provided in the input data.
    ...
    rt = type(data)  # return type will match input type
    if data.size == 0:  # return early if empty
        data = np.recarray((0,), dtype=MpPathlineFile.dtypes[7])
        return pd.DataFrame(data) if rt == pd.DataFrame else data
    if not isinstance(data, pd.DataFrame):  # convert to dataframe if needed
        data = pd.DataFrame(data)

    # do the conversion if we recognize the input dtype, otherwise raise an error
    if all(
        n in data.dtypes for n in MpPathlineFile.dtypes[7].names
    ):  # already in MP7 format?
        return data if rt == pd.DataFrame else data.to_records(index=False)
    elif all(n in data.dtypes for n in MpPathlineFile.dtypes[6].names):
        # todo stressperiod/timestep, from tdis
        # nper = len(tdis)
        data = np.core.records.fromarrays(
            [
                ...
                np.zeros(data.shape[0])
                if grid is None
                else data["k"] * grid.nrow * grid.ncol
                + data["i"] * grid.ncol
                + data["j"],
                data["xloc"],
                data["yloc"],
    ...
    elif all(n in data.dtypes for n in MpPathlineFile.dtypes[5].names) or all(
        n in data.dtypes for n in MpPathlineFile.dtypes[3].names
    ):
        # todo stressperiod/timestep, from tdis
        # nper = len(tdis)
        data = np.core.records.fromarrays(
    ...
    elif all(n in data.dtypes for n in PrtPathlineFile.dtypes["base"].names):
        # assign a unique particle index column, incrementing an integer
        # for each unique combination of irpt, iprp, imdl, and trelease.
        data = data.sort_values(PrtPathlineFile.key_cols)
        data["sequencenumber"] = data.groupby(
            PrtPathlineFile.key_cols
        ).ngroup()
        data = np.core.records.fromarrays(
            [
                data["sequencenumber"],
    ...
    elif all(n in data.dtypes for n in ParticleTrackFile.dtypes["base"].names):
        data = np.core.records.fromarrays(
            [
                data["particleid"],
                np.zeros(data.shape[0]),
        ...
        raise ValueError(
            ...
            f"{pformat(ParticleTrackFile.dtypes['base'].names)} (minimal expected dtype names)"
        )
...
def to_mp7_endpoints(
    ...
    """
    Convert pathline or endpoint data to MODPATH 7 endpoint format.
    ...
    ----------
    data : np.recarray or pd.DataFrame
        Tabular pathline or endpoint data, in MODPATH or MODFLOW 6 PRT format.
    grid : flopy.discretization.grid.Grid
        Optional grid discretization to compute local coordinates,
        if not provided in the input data.
    tdis : list of tuples (int, int, float)
    ...
    rt = type(data)  # return type will match input type
    if data.size == 0:  # return early if empty
        data = np.recarray((0,), dtype=MpEndpointFile.dtypes[7])
        return pd.DataFrame(data) if rt == pd.DataFrame else data
    if not isinstance(data, pd.DataFrame):  # convert to dataframe if needed
        data = pd.DataFrame(data)
    ...
    if all(
        ...
        return data if rt == pd.DataFrame else data.to_records(index=False)
    elif all(n in data.dtypes for n in MpEndpointFile.dtypes[3].names):
        raise NotImplementedError("MP3 endpoint files not yet supported")
    elif all(n in data.dtypes for n in MpEndpointFile.dtypes[5].names):
        raise NotImplementedError("MP5 endpoint files not yet supported")
    elif all(n in data.dtypes for n in MpEndpointFile.dtypes[6].names):
        raise NotImplementedError("MP6 endpoint files not yet supported")
    elif all(n in data.dtypes for n in PrtPathlineFile.dtypes["base"].names):
        # assign a unique particle index column incrementing an integer
        # for each unique combination of irpt, iprp, imdl, and trelease
        ...
        conditions = [
            ...
            for _, row in startpts.iterrows()
        ]
...
def to_prt_pathlines(
    ...
    """
    Convert MODPATH pathline or endpoint data to MODFLOW 6 PRT pathline format.

    todo: support mp timeseries input

    Parameters
    ----------
    data : np.recarray or pd.DataFrame
        MODPATH pathline or endpoint data
    ...
    rt = type(data)  # return type will match input type
    if data.size == 0:  # return early if empty
        data = np.recarray((0,), dtype=PrtPathlineFile.dtypes["base"])
        return pd.DataFrame(data) if rt == pd.DataFrame else data
    if not isinstance(data, pd.DataFrame):  # convert to dataframe if needed
        data = pd.DataFrame(data)

    # do the conversion if we recognize the input dtype, otherwise raise an error
    if all(n in data.dtypes for n in MpPathlineFile.dtypes[7].names) or all(
        n in data.dtypes for n in MpEndpointFile.dtypes[7].names
    ):
        data = np.core.records.fromarrays(
            [
            ...
            dtype=PrtPathlineFile.dtypes["base"],
        )
    elif all(n in data.dtypes for n in MpPathlineFile.dtypes[3].names) or all(
        n in data.dtypes for n in MpEndpointFile.dtypes[3].names
    ):
        raise NotImplementedError(
            "MP3 pathline and endpoint files not yet supported"
        )
    elif all(n in data.dtypes for n in MpPathlineFile.dtypes[5].names) or all(
        n in data.dtypes for n in MpEndpointFile.dtypes[5].names
    ):
        raise NotImplementedError(
            "MP5 pathline and endpoint files not yet supported"
        )
    elif all(n in data.dtypes for n in MpPathlineFile.dtypes[6].names) or all(
        n in data.dtypes for n in MpEndpointFile.dtypes[6].names
    ):
        raise NotImplementedError(
            "MP6 pathline and endpoint files not yet supported"
        )
    elif all(
        n in data.dtypes for n in PrtPathlineFile.dtypes["base"].names
    ):  # already in prt format?
        return data if rt == pd.DataFrame else data.to_records(index=False)
    elif all(n in data.dtypes for n in ParticleTrackFile.dtype["base"].names):
        data = np.core.records.fromarrays(
            [
                np.zeros(data.shape[0]),
                np.zeros(data.shape[0]),
