...
class ZoneBudget2(object):
    ...
    >>>bud.get_records()
    """
    def __init__(self, cbc_file, z, kstpkper=None, totim=None, aliases=None, mult=None):
        """
    ...
    def _compute_budget(self):
        ...
        if self.mult is not None:
            self._multiply_records(self.mult)
        return
    ...
    def _multiply_records(self, mult):
        for f in self._zonefieldnames:
            a = np.array([r for r in self.recordarray[f]]) * mult
            self.recordarray[f] = a
        return
