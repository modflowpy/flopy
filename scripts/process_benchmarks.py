...
def get_benchmarks(paths):
    ...
    for path in paths:
        with open(path, 'r') as file:
            ...
            if len(python.split('.')) == 3: python = python.rpartition('.')[0]
            tstamp = jsn['datetime']
            ...
            for benchmark in bmarks:
                ...
                for stat, value in benchmark['stats'].items():
                    if stat not in included: continue
                    benchmarks.append({
                        "system": system,
                        "python": python,
                        "time": tstamp,
...
def matplotlib_plot(stats):
    ...
    for system in systems:
        for python in pythons:
            patches.append(Line2D([0], [0], color=colors[python], marker=markers[system], label=f"{system} Python{python}"))
    leg = plt.legend(handles=patches, loc='upper left', ncol=3, bbox_to_anchor=(0, 0), framealpha=0.5, bbox_transform=ax.transAxes)
...
def seaborn_plot(stats):
    ...
    for i, (stat_name, stat_group) in enumerate(stats):
        ...
        sp = sns.scatterplot(x='time', y='value', style='system', hue='python', data=stat_df, ax=ax, palette="YlOrBr")
        sp.set(xlabel=None)
        ax.set_title(stat_name)
